...
export function setFilter(req, res) {
  const saveFilter = filterToSave => {
    filterToSave.save((err, filterUpdate, numAffected) => {
      if (err) throw err;
      res.send(filterUpdate);
    });
  };

  var { rs, type, domain, name, url, filter } = req.body.set; // GET PARAMS IN REQUEST
  ...
  if (filter && type && domain) {
    ...
    Filters.findOne({ domain: domain }, function(err, object) {
      // THROW ERROR
      if (err) throw err;
          ...
          case Constant.ANCHOR_CODE:
            {
              contents = contents || [];
              if (contents.length) {
                var hasf_Item = contents.some(item => {
                  return item.url === url;
                });
                if (hasf_Item) {
                  contents.map(item => {
          ...
          case Constant.CONTAINERS_CODE:
            {
              ...
              if (category !== undefined) {
                containers.category.push({ url, filter: category });
              }
            }
            break;
          default:
